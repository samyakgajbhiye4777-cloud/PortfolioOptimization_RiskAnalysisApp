...
def fetch_stock_data_robust(tickers,start_date, end_date):
    ...
    for ticker in tickers:
        try:
            ...
            # hist_data = stock.history(period=period) --- Actual code
...
class PortfolioOptimizer:
    def __init__(self, tickers, start_date, end_date, rf=0.055):
...
with st.sidebar:
    ...
    mode = st.radio("Select Data Range Type:", ["Historical Period", "Custom Range"], index=0)
    today = date.today()
    end_date = today 
    if mode == "Historical Period":
        period = st.selectbox("Historical Period",
                              options=["6mo", "1y", "2y", "3y", "5y"],
                              index=2)
        
        # Map period to months
        months_map = {"6mo": 6, "1y": 12, "2y": 24, "3y": 36, "5y": 60}
        months = months_map[period]

        # Calculate start and end dates automatically
        start_date = end_date - relativedelta.relativedelta(months=months)
    else:
        period = None
        st.info("Please select your custom date range below:")
    ...
    # # period = st.selectbox("Historical Period", 
    #                      options=["6mo", "1y", "2y", "3y", "5y"], ----Actual code
    #                      index=2)
...
if run_analysis and tickers_input:
    ...
    with st.spinner("🔄 Fetching data and computing metrics..."):
        optimizer = PortfolioOptimizer(formatted_tickers, start_date,end_date, rf=rf_rate)
